...
IMG ?= quay.io/giantswarm/fleet-membership-operator-gcp:dev
...
deploy-acceptance-cluster: docker-build create-acceptance-cluster deploy-capg-crds create-test-secrets deploy-crds-on-workload deploy
...
render: architect
	...
	cp -r $(shell pwd)/helm/fleet-membership-operator-gcp $(shell pwd)/helm/rendered/
	$(ARCHITECT) helm template --dir $(shell pwd)/helm/rendered/fleet-membership-operator-gcp
...
deploy: manifests render ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	KUBECONFIG="$(KUBECONFIG)" helm upgrade --install \
		...
		--wait \
		fleet-membership-operator-gcp helm/rendered/fleet-membership-operator-gcp
